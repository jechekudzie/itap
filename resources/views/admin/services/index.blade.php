            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0" id="page-title">Services</h4>
                ...
                <div class="row">
                                ...
                                <table style="width: 100%;" id="buttons-datatables"
                                    ...
                                    <tr>
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="buttons-datatables"
                                            rowspan="1" colspan="1"
                                            aria-label="Position: activate to sort column ascending"
                                            style="width: 336.4px;">Service Packages
                                        </th>
                                    ...
                                    @foreach($services as $service)
                                        <tr class="even">
                                            ...
                                            <td>
                                                <a href="{{route('service-packages.index', $service->slug)}}" class="btn btn-sm btn-primary" title="Service Packages">
                                                    Packages ({{$service->servicePackages->count()}})
                                                </a>
                                            <td>
                                                <!-- Edit Button -->
                                                <a href="javascript:void(0);" class="edit-button btn btn-sm btn-primary" data-name="{{ $service->name }}" data-slug="{{ $service->slug }}" data-category="{{ $service->service_category_id }}" title="Edit">
                                                    <i class="fa fa-pencil"></i>
                                                </a>

                                                <!-- Delete Button -->
                                                <form action="{{ route('services.destroy', $service->slug) }}" method="POST" onsubmit="return confirm('Are you sure?');" style="display: inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                    ...
                    <div class="col-xxl-3">
                        <div class="card border card-border-light">
                            <div class="card-header">
                                <h6 id="card-title" class="card-title mb-0">Add  Services</h6>
                            </div>
                            <div class="card-body">
                                <form id="edit-form" action="{{route('services.store')}}" method="post" enctype="multipart/form-data">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.edit-button').on('click', function() {
                ...
                $('#card-title').text('Edit - ' + name + ' Services');
                $('#page-title').text('Edit - ' + name + ' Services');
            });
            ...
            $('#new-button').on('click', function() {
                ...
                submitButton.text('Add New');
                $('#name').val('');
                $('#card-title').text('Add Services');
                $('#page-title').text('Add New Services');
            });
        });
